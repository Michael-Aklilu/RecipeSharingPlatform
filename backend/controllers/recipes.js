...
recipeRouter.get('/:id', async (req,res) => {
  try{
    ...
    console.log(error)
    res.status(500).json({error: 'An error occurred while fetching a recipe'})
  }
...
recipeRouter.put('/:id',userExtractor, async (req,res) => {
   try{
      ...
      const newRecipe = {title, description,ingredients,instructions,servings,prepTime,cookTime,imageUrl,RegisteredUser}

      const updatedRecipe = await Recipes.findByIdAndUpdate(id,newRecipe,{new:true}).populate('RegisteredUser')
      ...
      if (recipeIndex === -1) {
        return res.status(404).json({ error: 'Recipe not associated with this user' });
      }
...
recipeRouter.delete('/:id',userExtractor, async (req,res) => {
    try{
      ...
      user.addedRecipes = user.addedRecipes.filter((recipe) => recipe.toString() !== id)
